...
- manifest.json：应用元数据、平台配置与权限声明
- uni.promisify.adaptor.js：Promise 化拦截器，统一多平台异步调用风格
...
- manifest.json：定义应用名称、版本、平台特定配置（app-plus、mp-weixin 等）、权限声明等
- uni.promisify.adaptor.js：为 uni 原生 API 的回调风格结果包裹 Promise，统一异步调用风格，提升跨平台一致性
...
Inter-->>App : 转换为Promise并resolve/reject
```
...
Mf["manifest.json"] --> Build["构建/分发流程"]
U["uni.promisify.adaptor.js"] --> Api["api/*"]
Pg1 --> Api
Pg2 --> Api
Pg3 --> Api
Pg4 --> Api
...
- manifest.json 决定应用元数据与平台能力，特别是 Android 权限与分发配置
- uni.promisify.adaptor.js 提升了多平台 API 的一致性，简化了异步处理逻辑
...
- app-plus.compilerVersion：编译器版本（可选）
- app-plus.splashscreen：启动屏配置（可选）
- app-plus.distribute.android.permissions：Android 权限列表（可选）
- mp-weixin/mp-alipay/mp-baidu/mp-toutiao：小程序平台配置（可选）
- uniStatistics.enable：统计开关（可选）
- vueVersion：Vue 版本（可选）
